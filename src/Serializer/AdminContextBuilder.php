...
final class AdminContextBuilder implements SerializerContextBuilderInterface
{
    ...
    public function createFromRequest(Request $request, bool $normalization, ?array $extractedAttributes = null): array
    {
        ...
        $isSuperAdmin = $this->authorizationChecker->isGranted(User::ROLE_SUPER_ADMIN);
        $isAdmin = $this->authorizationChecker->isGranted(User::ROLE_ADMIN);
        $isCompanyAdmin = $this->authorizationChecker->isGranted(User::ROLE_COMPANY_ADMIN);

        if ($isSuperAdmin) {
            $context['groups'][] = $normalization ? 'super:admin:read' : 'super:admin:write';
        }
        ...
        if ($isCompanyAdmin) {
            $context['groups'][] = $normalization ? 'company:admin:read' : 'company:admin:write';
        }
    ...
    private function addDefaultGroups(array $context, bool $normalization)
    {
        ...
        $operationName = $context[$itemOrCollection.'_operation_name'];
        return [
            // {class}:{read/write}
            // e.g. user:read
